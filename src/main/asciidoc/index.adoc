...
예를 들어, 잘못된 요청으로 계정를 만들려고 했을 때 다음과 같은 `400 Bad Request` 응답을 받는다.
...
계정 리소스는 계정를 만들거나 조회할 때 사용한다.

[[resources-accounts-list]]
=== 계정 목록 조회

`GET` 요청을 사용하여 서비스의 모든 계정를 조회할 수 있다.
...
`Get` 요청을 사용해서 기존 계정 하나를 조회할 수 있다.
